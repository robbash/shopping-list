  ...
  Api.addRoute('alexa', { authRequired: false }, {
    post() {
      ...
      if (request.type === 'LaunchRequest') {
        response.response.outputSpeech.text = 'Ich höre...'
        ...
        switch (intent.name) {
          case 'ROBBASH_add':
            ...
            if (itemExists(item.title)) {
              response.response.outputSpeech.text = `${item.title} steht bereits auf der Liste`
            } else if (Items.insert(item)) {
              response.response.outputSpeech.text = 'Okay!'
            } else {
              response.response.outputSpeech.text = 'Es ist leider ein Problem aufgetreten!'
            }
          ...
          case 'ROBBASH_list':
            ...
            if (items.length > 1) {
              response.response.outputSpeech.text = `${items.slice(0, -1).join(', ')} und ${items.slice(-1)}`
            } else if (items.length > 0) {
              response.response.outputSpeech.text = items[0]
            } else {
              response.response.outputSpeech.text = 'Die Liste ist leer...'
            }
          ...
          case 'ROBBASH_check':
            ...
            response.response.outputSpeech.text = found ? 'ja' : 'nein'
